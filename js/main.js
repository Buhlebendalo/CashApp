...
$(document).ready(function() {
    ...
    function newActivity() {
        ...
        var avatars = ['https://i.imgur.com/Wxa07tk.png',
            'https://i.imgur.com/Wxa07tk.png'
        ];

        var flag = flags[Math.floor(Math.random() * flags.length)];
        var avatar = avatars[Math.floor(Math.random() * avatars.length)];
        var user = users[Math.floor(Math.random() * users.length)];
        var vBucksRand = Math.floor((Math.random() * 9000) + 1000);;
        ...
        $('.activityContent').hide().prepend('<div class="live-stats">' +
            ...
            '<li><img class="item-gen" src="img/vBucks.png" alt="" />Cash: <span class="bucks-color">' + vBucksRand + '</span> </li>' +
            '</ul>' +
            '</div>' +
            '</div>').fadeIn();
    }
    ...
    function progressBar() {
        ...
        function frame() {
            ...
            if (width == 30) $('.load-textJS').html('Connection successfull!');
            if (width == 40) $('.load-textJS').html('Fetching data...');
            if (width == 50) $('.load-textJS').html('Getting IP Address...');
            if (width == 60) $('.load-textJS').html('Getting Username...');
            if (width == 70) $('.load-textJS').html('Rerouting...');

            if (width == 80) $('.load-textJS').html('Posting data...');
